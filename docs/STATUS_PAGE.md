...
사이드바 탭 2개 — **개요 / 상세**. 상세 탭 안에 백엔드 · E2E · 앱 에러 세 구역을 쌓고,
각 구역은 제목 오른쪽에 자기 상태를 단다. 탭 옆의 점은 **그 탭이 담은 것 중 가장 나쁜 상태**다
(백엔드만 보고 초록을 띄우면 E2E가 깨져 있어도 사이드바가 조용해진다). 보던 탭은
`localStorage`에 기억한다 — 페이지가 5분마다 자동 새로고침되기 때문이다.

색·타이포는 `packages/tokens`의 시맨틱 토큰을 그대로 CSS 변수로 옮겼다(`--text-secondary`,
`--surface-muted`, `--ok/--warn/--bad` 등). **라이트 전용**이다 — 다크 모드는 두지 않는다.
본문 글꼴은 디자인 시스템과 같은 **Pretendard**를 CDN에서 받고, 실패하면 `-apple-system`으로
폴백한다 — 글꼴은 소프트 의존이라 못 받아도 페이지는 그대로 뜬다.

요약과 표는 **카드**에 담는다. 숫자는 고정폭에 우측 정렬이고, 한글 헤더에는 고정폭을 쓰지
않는다(어색하게 벌어진다).

**색 사용 규칙**이 하나 있다:

- 화면의 색 축은 **primary(파랑)**. 선택된 탭, 알림 줄처럼 UI 요소가 쓴다.
- **초록·빨강은 성공·실패 값에만** 쓴다 — 가용률, 통과 수, 실패 시각, 상태 단어(정상/이상/실패).
  그 밖의 요소가 초록·빨강을 쓰면 정작 봐야 할 수치가 묻힌다.
- 가동 띠의 정상 구간도 회색이다. 정상을 색으로 칠하면 화면에서 가장 큰 요소가 아무 정보도
  없는 띠가 되고, 빨간 실패 칸이 그 안에 묻힌다.

엔드포인트 URL은 화면에 싣지 않는다. 서비스 이름만으로 충분하고, 공개 페이지에 내부 주소를
나열할 이유가 없다.
...
- **cron은 정시를 보장하지 않는다.** GitHub 스케줄은 부하에 따라 지연·누락된다.
  도입 직후 `*/30`으로 두었을 때 **4시간 넘게 한 번도 실행되지 않았다.** 스케줄 큐가
  매시 0분에 몰리기 때문으로 보여, 지금은 정시를 피해 `7,37`로 잡는다. 그래도 누락은
  남으므로 가용률은 정확한 SLA가 아니라 추세로 읽어야 한다.
- **점검이 멈추면 페이지가 스스로 말한다.** 마지막 기록이 90분(점검 주기 × 3회 누락)보다
  오래되면 상태를 `정상`이 아니라 **`점검 중단`**으로 바꾸고, 상단에 경고 줄을 띄우며
  가용률을 회색으로 낮춘다. 사이드바에는 **페이지 갱신 시각과 마지막 점검 시각을 나란히**
  둔다 — 둘이 어긋나 있다는 것 자체가 신호다.

  이게 없으면 점검이 멈춘 동안 마지막 기록이 성공이었다는 이유로 화면이 계속 "정상"을
  띄운다. 서버가 멀쩡한 것과 감시가 멈춘 것은 전혀 다른 상태다.
- **스케줄 워크플로는 저장소가 60일간 조용하면 자동 비활성화된다.** 이 경우도 위의
  `점검 중단` 표시로 드러난다.
